...
* **Topics**:
  ...
  * `iris/observations/v1`: Nodes broadcasting their fetched imagery and TLS proofs.
...
* **Language**: Python (invoked via Rust FFI or running as an isolated local microservice via gRPC).
* **Libraries**: `rasterio` (for GeoTIFF parsing), `numpy` / `PyTorch` (for tensor math), `pyproj` (for coordinate transformations).
...
* `Observing`: Fetching API data, generating TLS proofs, and gossiping to `iris/observations/v1`.
* `Aggregating`: The Leader calculates similarities.
* `Pre-Commit`: The Leader broadcasts the proposed Average Scenario.
* `Commit`: Nodes verify the proposal and broadcast signature shares.
...
* **Key Generation (DKG)**: When a committee is formed, run a Distributed Key Generation protocol so the committee shares a single aggregate Public Key, and each node holds a private key share.
